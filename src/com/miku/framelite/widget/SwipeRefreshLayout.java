...
package com.miku.framelite.widget;
...
public class SwipeRefreshLayout extends ViewGroup {
	...
	public static final int MODE_SWIPE_DOWN=0;
	public static final int MODE_SWIPE_UP=1;
	
	private static final long RETURN_TO_ORIGINAL_POSITION_TIMEOUT = 300;
	private static final float ACCELERATE_INTERPOLATION_FACTOR = 1.5f;
	private static final float DECELERATE_INTERPOLATION_FACTOR = 2f;
	private static final float PROGRESS_BAR_HEIGHT = 5;
	...
	private int mSwipeMode=MODE_SWIPE_UP;
	 ...
	 * 
	 * @param colorRes1
	 *            Color resource. while Progress >= 25 && Progress < 50
	 * @param colorRes2
	 *            Color resource. while Progress >= 50 && Progress < 75
	 * @param colorRes3
	 *            Color resource. while Progress >= 75 && Progress < 100
	 * @param colorRes4
	 *            Color resource. while Progress >= 0 && Progress < 25
	 */
	 ...
	 * 
	 * @param color1
	 *            Color. while Progress >= 25 && Progress < 50
	 * @param color2
	 *            Color. while Progress >= 50 && Progress < 75
	 * @param color3
	 *            Color. while Progress >= 75 && Progress < 100
	 * @param color4
	 *            Color. while Progress >= 0 && Progress < 25
	 */
	...
	public boolean canChildScrollDown() {
		if (android.os.Build.VERSION.SDK_INT < 14) {
			if (mTarget instanceof AbsListView) {
				final AbsListView absListView = (AbsListView) mTarget;
				return absListView.getChildCount() > 0
						&& (absListView.getLastVisiblePosition() < absListView.getCount()-1 || absListView.getChildAt(absListView.getChildCount() - 1)
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		//need to fixed
		boolean canTrigger=mSwipeMode==MODE_SWIPE_DOWN?!canChildScrollUp():!canChildScrollDown();
		if (isEnabled() && !mReturningToStart && canTrigger) {
...
//							}
	...
	private void updateContentOffsetTop(int target) {
		final int current =mTarget.getTop();
		if (target > mDistanceToTriggerSync) {
			target = (int) mDistanceToTriggerSync;
		} else if (target < 0) {
			target = 0;
		}
		target=mSwipeMode==MODE_SWIPE_DOWN?target:-target;
